...
JAVACC=javac
JAR=jar
DIR_CLASSES=SNCF_classes  # a dir to store classes and not mix too many files with sources


all: SNCF.jar

wordcount.jar: SNCF.class 
	mkdir -p $(DIR_CLASSES)
	$(JAR) -cvf $@ -C $(DIR_CLASSES) .
	@echo "* JAR READY *"
	@echo "-> run with 'hadoop jar $@ SNCF input output' (make sure you have 'input' and 'output' does not exist yet)"

SNCF.class: SNCF.java
